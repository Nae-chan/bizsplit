...
describe("shopify sync", () => {
  ...
  it("creates a connection with encrypted credentials", async () => {
    ...
    const conn = await createConnection({
      ...
      webhookSecret: "shpss_secret",
    });
    connectionId = conn.id;
    expect(conn.shopName).toBe("Ripright");

    const [row] = await testDb.select().from(schema.storeConnection);
    expect(row.encryptedAccessToken).not.toContain("shpat_secret_token");
    const { decryptSecret } = await import("@/lib/crypto");
    expect(decryptSecret(row.encryptedAccessToken)).toBe("shpat_secret_token");
  });
